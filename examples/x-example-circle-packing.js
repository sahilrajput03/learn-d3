...
function Pack(
	data,
	{
		...
		label = fn, // given a leaf node d, returns the display name
		title = fn, // given a node d, returns its hover text
	...
	if (L) {
		...
		const leaf = node.filter((d) => !d.children && d.r > 10 && L[d.index] != null)
